...
let raytraced_image () =
  ...
  let camera_center = 0.0, 0.0, 0.0 in
  ...
  let pixel_delta_td = Vec3d.(viewport_td / float_of_int image_height) in
  (* Calc the location of the upper left pixel *)
  let viewport_upper_left =
    Vec3d.(
      camera_center - (0., 0., focal_length) - (viewport_lr / 2.0) - (viewport_td / 2.0))
  in
  let pixel00_loc =
    Vec3d.(viewport_upper_left + (0.5 * (pixel_delta_lr + pixel_delta_td)))
  in
  let render ~row ~col =
    let pixel_center =
      Vec3d.(
        ...
        + (float_of_int row * pixel_delta_td))
    in
